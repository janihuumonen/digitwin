...
<body>
  ...
  <form action="/save" method="post">
	...
	<label for="token">Palvelimen auth token:</label><br>
...
def run_main(o):
	...
	while True:
		...
		if ((prevheight != height) or (abs(lastSentTemp-temp) > 0.2)) and o.get('url'):
			# Trigger notification
			lastSentTemp = tempsensor.val
			data = {
				'token': o.get('token',''),
				'height': str(heightsensor),
				'temp': str(tempsensor)
			}
			send(o['url'], json.dumps(data))
